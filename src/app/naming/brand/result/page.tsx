...
function BrandResultPageInner() {
    const searchParams = useSearchParams();
    const input = {
        industry: searchParams.get("industry") ?? "general",
        keywords: searchParams.get("keywords") ?? "Brand",
        ...
        langPref: searchParams.get("langPref") ?? undefined,
        vibePref: searchParams.get("vibePref") ?? undefined,
    };
                    ...
                    {results.map((result, idx) => (
                        <div key={idx} className="bg-white rounded-3xl p-0 shadow-lg shadow-gray-200/50 border border-gray-100 overflow-hidden ring-1 ring-gray-50">
                                ...
                                <div className="space-y-4">
                                    <div className="flex gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-800 leading-relaxed">{result.meaning}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-800 leading-relaxed">{result.marketingPoint}</p>
                                        </div>
                                    </div>
                                    ...
                                    {result.sajuMatch && (
                                        <div className="bg-yellow-50 rounded-xl p-3 border border-yellow-100 flex gap-3 items-start">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-yellow-700 leading-snug">{result.sajuMatch.desc}</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="bg-gray-50 p-4 border-t border-gray-100 flex items-center justify-between">
                                <VerificationItem label="도메인" status={result.verification.domain.available} warning={result.verification.domain.reason} />
                                <div className="h-8 w-px bg-gray-200"></div>
                                <VerificationItem label="상표권" status={result.verification.trademark.available} warning={result.verification.trademark.reason} />
                                <div className="h-8 w-px bg-gray-200"></div>
                                <VerificationItem label="SNS" status={result.verification.sns.available} />
                            </div>
                        </div>
                    ))}
                </div>
...
export default function AdvancedBrandResultPage() {
    return (
        <Suspense fallback={<div className="min-h-screen flex items-center justify-center">로딩 중...</div>}>
            <BrandResultPageInner />
        </Suspense>
    );
}
